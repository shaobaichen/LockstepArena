...
### 2026-09-14 correction and re-verification after independent review

The preceding evidence is retained as historical evidence for `9f5eeaeb0434ef80e818727d08eaddc5d63e5d3a`; it is not substituted for the following post-correction runs. Normal correction commits `df82dfab311283d59882f8f6fe2f1499a2904d4c` and `d396498cf0f233112e8fbd87667c6f1c1f4fe1e4` remain on the same Gate14 branch. Their complete changed-file set is `Packages/com.locksteparena.client-demo/Runtime/TcpDemoClient.cs`, `Server/LockstepArena.Server.DemoHost/DemoSession.cs`, `Server/LockstepArena.Server.DemoHost/TcpDemoServer.cs`, `Assets/LockstepArenaDemo/Runtime/LockstepArenaDemoController.cs`, `Assets/LockstepArenaDemo/Tests/Editor/UnityDemoPresentationTests.cs`, `Tests/LockstepArena.DemoFlow.Tests/BattlePreparationTests.cs`, `Tests/LockstepArena.DemoFlow.Tests/Gate14DemoGoldenVector.cs`, and `Tests/LockstepArena.DemoFlow.Tests/SessionRoomTests.cs`.

- CONTROL `BeginConnect` now initiates a nonblocking IPv4 candidate and stays in `ConnectingControl` until a later bounded `PumpOnce` observes completion. `BattlePreparing` similarly initiates, but does not synchronously complete, BATTLE connection; bounded attachment progress completes it before ticket send. No Task, async, Thread, Timer, or background scheduler was added.
- Network-originated control commands now prepare candidate state and all serialized events, preflight bounded destination capacity, then commit domain state and approved bytes. Intermediate session event storage is bounded and event transfer peeks before dequeue. A capacity failure leaves membership/readiness/lifecycle/BattleId/ticket and required events unconsumed for deterministic retry; no unbounded queue or larger configured capacity was introduced.
- Named control connections are processed in ascending `SessionId` order; unnamed connections use a stable accept ordinal. Existing Test37 proves order even when acceptance and session assignment differ, as well as both bounded nonblocking connection paths. Existing room/readiness/Start tests exercise backpressure; the Gate14 registry remains exactly `48/48`.
- The Debug panel now displays `Rejection=`. The client maps the approved rejection cases to stable `NOT_HOST` and `NOT_READY` text. The existing .NET rejection assertions and Unity presentation test were strengthened without adding test names. The first correction RED run reported `43/48`, then GREEN `48/48`; the display correction RED run reported `46/48`, then GREEN `48/48`.

After the final source correction, restore-assets preflight found no missing project assets. All **24** approved sequential Release builds were rerun from build 1 and each reported **0 warnings / 0 errors**. Fresh dependency-free executions passed Gate3 `38/38`, Gate4 `32/32`, Gate5 `35/35`, Gate6 `24/24`, Gate7 `32/32`, Gate8 `8/8`, Gate9 `27/27`, Gate10 `27/27`, Gate11 `32/32`, Gate12 `36/36`, Gate13 `49/49`, and Gate14 `48/48`. The real-TCP suites ran under an external 30-second process watchdog without triggering it. The Gate3 Server Golden again reported Tick 1000, four players, digest `89A7DD66F8D9E871`. After the manual run, a further focused Gate14 execution again reported `RESULT 48/48 passed`, including all four actual-Golden/alternate-segmentation tests and the strengthened bounded-pump/transaction tests.

Pinned regeneration again used Grpc.Tools `2.83.0`, bundled `protoc` `libprotoc 35.1` at `C:\Users\张晨旭\.nuget\packages\grpc.tools\2.83.0\tools\windows_x64\protoc.exe`, with no `PROTOBUF_PROTOC` or `Protobuf_ProtocFullPath` override. Schema and generated-source SHA-256 values were unchanged, the physical counts remained one `.proto` and one `.g.cs`, and regeneration yielded no schema/generated Git diff.

Unity `6000.3.10f1` separately produced new NUnit XML under `.artifacts/verification/gate14-unity-final/`: Gate14 Demo `3/3`, Gate12 Prediction `2/2`, Gate7 StreamFraming `1/1`, Gate5 Protocol `2/2`, and Gate3 named Golden `1/1`; all required named cases were `Passed` with zero failures. XML, not Unity exit code, was the pass oracle. No Assets/ProjectSettings serialization diff remained after these runs. A new Windows Player was then successfully built from `Assets/LockstepArenaDemo/Scenes/LockstepArenaDemo.unity` to `.artifacts/Gate14DemoPlayerCorrection/LockstepArenaDemo.exe`; the older Player was not overwritten. The one temporary `EditorBuildSettings.asset` scene selection and five individually inspected Unity build-serialization changes were restored at exact worktree-local paths; the worktree was clean afterward.

The refreshed real Player (`Bravo`) plus Editor (`Alpha`) manual run supplied direct UI captures of `Rejection=NOT_HOST` after Bravo attempted Start and `Rejection=NOT_READY` after ready Alpha attempted Start while Bravo remained unready. A supplied Alpha settlement capture shows State/authority/predicted Tick `4`, Replay `4`, cumulative Dirty `4`, equal authority/predicted digest `D8E54FF828A4C670`, and `SettlementVerified=True`. The operator reported the final-state check complete and later confirmed stopping the demo; the Bravo final-state screen was not separately captured in this correction run. The separately rerun actual-Golden tests assert both clients' Tick, digest, Dirty `[true,true,true,true]`, settlement, Return, Exit, and alternate segmentation. After the operator stopped DemoHost, neither loopback demo port had a listener.

Final committed-diff and worktree audits again found protected Gate3–13 production/test paths, `Packages/manifest.json`, and `ProjectSettings` unchanged relative to frozen Gate13 `57243ae82ff87a5f51dc86d34342d697963b34dc`. The migrated Gate13 blobs remained byte-identical (`da717a9674d603b9f20a2561a445e290bb830bf3` and `d43ff4709b7e61bdca9faeeb406400332a26128e`), with one physical copy each. The exact three `.gitignore` exceptions, one schema/one generated source, no tracked symlinks, no package build artifacts, no forbidden Gate14 source symbols, and `git diff --check` were reconfirmed. The ordinary checkout still contained only its two user-owned `Assets/Settings/Mobile_RPAsset.asset` and `ProjectSettings/ShaderGraphSettings.asset` modifications.

### 2026-09-14 buffered-command and physical CONTROL-loss micro-correction

Correction commit `93502fed94fb564a861b65febdeeecde529f7695` changes only `TcpDemoServer.cs` and the existing `SessionRoomTests.cs` / `BattlePreparationTests.cs`. Before the production correction, the strengthened existing tests produced `45/48`: the retained command was not retried without new network bytes, and backpressured survivor events vetoed both open-host-loss and battle-abort cleanup. After the correction, the exact registry produced `48/48` without adding a test name.

Each server pump still performs at most one readiness-guarded bounded CONTROL read. It now processes up to `MaxControlMessagesPerPump` already-buffered commands even when that read returns no new payload. The test sends exactly one framed `CreateRoom` command under bounded outbound backpressure, sends no further client bytes, observes no premature room creation, then observes exactly one processed command, Room 1, and its authoritative `RoomSnapshot` after bounded send progress. Capacity failure retains the queue head. Normal network-originated commands continue to preflight all recipients before domain commit.

For physical CONTROL loss, mandatory room removal or battle invalidation/abort now precedes independent survivor notification. A survivor whose bounded control queue cannot accept `LobbyEntered` or Aborted settlement is fail-stopped and removed instead of remaining a divergent live session. Existing tests now cover a backpressured survivor on open-host loss and on both PreparingBattle and InBattle participant CONTROL loss; they verify room removal, ticket/preparation invalidation, and closed sessions. No unbounded queue, retry loop, or Gate13 source change was introduced.

After this source commit, restore assets were present for all 24 projects. The complete approved sequential Release build matrix completed `24/24`, each with `0 warnings / 0 errors`. Fresh .NET executions passed Gate3 `38/38`, Gate4 `32/32`, Gate5 `35/35`, Gate6 `24/24`, Gate7 `32/32`, Gate8 `8/8`, Gate9 `27/27`, Gate10 `27/27`, Gate11 `32/32`, Gate12 `36/36`, Gate13 `49/49`, and Gate14 `48/48`; the real-TCP suites used an external bounded process watchdog without triggering it. A further Gate14 execution after Unity again returned `48/48`. The four Gate14 Golden/alternate-segmentation cases passed; both client Dirty sequences remain `[true,true,true,true]`, State Tick is `4`, and final authority/prediction digest remains `D8E54FF828A4C670`. Gate3 Server Verification again reported Tick 1000, four players, digest `89A7DD66F8D9E871`.

Pinned Protocol regeneration used Grpc.Tools `2.83.0` and bundled `protoc` `libprotoc 35.1`, with no protoc environment override, exactly one `.proto` and one `.g.cs`, and zero schema/generated Git diff. Their SHA-256 values remained `1219FA6EB5CBB6506283523675F22FC65F08271F4FA62D39DF33CCA13C6A952F` and `786B1454EEC2DC1DF1F9B263904833EC96FBC2114B70F0CEAE309F2A285B33D5` respectively.

Unity `6000.3.10f1` independently generated fresh NUnit XML under `.artifacts/verification/gate14-micro-correction/`: Gate14 Demo `3/3`, Gate12 Prediction `2/2`, Gate7 StreamFraming `1/1`, Gate5 Protocol `2/2`, and Gate3 named Golden `1/1`; all required named tests were `Passed` and each run had zero failures. The first attempts before the operator closed an already-open Editor on this same worktree exited before Test Runner and produced no XML; they were not counted as passes. The successful runs used the frozen assembly/filter arguments, no `-quit`, and `Start-Process -Wait`. After each run, `Assets` and `ProjectSettings` had no worktree-local diff.

Final scope audit relative to frozen Gate13 found zero changes in protected Simulation, ClientPrediction, StreamFraming, Server FrameSync, ProtocolAuthority, Server LiveTcp, `ProjectSettings`, `Packages/manifest.json`, and pre-existing test sources. The migrated Gate13 `TcpClientBattlePump.cs` and `PredictedTcpClientBattleRuntime.cs` blobs remain identical to their originals (`da717a9674d603b9f20a2561a445e290bb830bf3`, `d43ff4709b7e61bdca9faeeb406400332a26128e`). `.gitignore` retains exactly the three approved additions; there are no tracked symlinks, package-local build artifacts, package reparse points, or forbidden Gate14 source symbols. The ordinary checkout still holds only the two protected user-owned asset/settings modifications.

### 2026-09-14 aborted-settlement ReturnToLobby lifecycle correction

Correction commit `f1625353a8927f96257f3926da16c02a195c2140` changes only `Server/LockstepArena.Server.DemoHost/TcpDemoServer.cs` and the existing `Tests/LockstepArena.DemoFlow.Tests/SettlementLifecycleTests.cs` / `BattlePreparationTests.cs`. The strengthened existing battle-failure test first produced RED `47/48`: after a real CONTROL `ReturnToLobby` command, the surviving session remained in `Settlement` because its aborted room had already been removed. No test name was added. The correction clears aborted sessions' room ID, join ordinal, and readiness ownership while retaining `Settlement`; `ReturnSettledSession` preflights `LobbyEntered`, skips retained-room removal only for explicit `RoomId == 0`, then commits Lobby. Normal TickLimit settlement retains a nonzero room ID and continues through `RemoveSettledParticipant`. `CloseSession` likewise skips removed-room lookup only for the explicit aborted sentinel. Missing nonzero rooms are not accepted.

The same test now observes the Aborted settlement on a real survivor CONTROL connection, sends real `ReturnToLobby`, observes `LobbyEntered`, verifies Lobby and `RoomId == 0`, requests a fresh room list, and confirms the aborted room is absent. The existing physical CONTROL-loss test was also strengthened for an unpressured survivor in PreparingBattle: the host connection closes, preparation is invalidated, the room is removed, and the survivor's real CONTROL `ReturnToLobby` succeeds. Existing backpressured-survivor fail-stop assertions remain. Focused GREEN and a further post-Unity Gate14 run each reported `RESULT 48/48 passed`, including normal retained-room settlement and the four Gate14 Golden/alternate-segmentation tests.

Fresh restore-assets preflight found no missing assets. All 24 approved Release builds ran sequentially from build 1, each with `0 warnings / 0 errors`. Fresh .NET results were Gate3 `38/38`, Gate4 `32/32`, Gate5 `35/35`, Gate6 `24/24`, Gate7 `32/32`, Gate8 `8/8`, Gate9 `27/27`, Gate10 `27/27`, Gate11 `32/32`, Gate12 `36/36`, Gate13 `49/49`, and Gate14 `48/48`. Real-TCP Gates 8/11/13/14 used an external 120-second process watchdog without timeout. Server Verification again returned Tick 1000, four players, digest `89A7DD66F8D9E871`. The approved Gate14 actual-Golden assertions continue to require both Dirty sequences `[true,true,true,true]`, State Tick 4, initial/intermediate digests `D08BA63E403C71AF`, `F82D3BE4A98024B5`, `A4DAA5FBE7E940ED`, `7A653646579E6AEC`, and final digest `D8E54FF828A4C670` for server and both clients.

Pinned Protocol regeneration again used Grpc.Tools `2.83.0` with bundled `protoc` `libprotoc 35.1`; no protoc environment override was set. The tracked schema/generated counts remained one each, their SHA-256 values remained `1219FA6EB5CBB6506283523675F22FC65F08271F4FA62D39DF33CCA13C6A952F` and `786B1454EEC2DC1DF1F9B263904833EC96FBC2114B70F0CEAE309F2A285B33D5`, and schema/generated diff was clean.

Unity `6000.3.10f1` separately generated fresh NUnit XML under `.artifacts/verification/gate14-abort-return-unity/`: Gate14 Demo `3/3`, Gate12 Prediction `2/2`, Gate7 StreamFraming `1/1`, Gate5 Protocol `2/2`, and Gate3 named Golden `1/1`. Every required named test was `Passed` with zero failures; XML, not process exit, was the oracle. Each run left no Assets/ProjectSettings diff in the Gate14 worktree. Final scope audit found no committed diff in protected Gate3–13 production/test paths, manifest, or ProjectSettings relative to Gate13 baseline `57243ae82ff87a5f51dc86d34342d697963b34dc`. Both migrated Gate13 client runtime blobs remained byte-identical (`da717a9674d603b9f20a2561a445e290bb830bf3`, `d43ff4709b7e61bdca9faeeb406400332a26128e`). `.gitignore` still has exactly three approved additions, and packages contain no build binaries, tracked symlinks, or reparse points. The ordinary checkout still contains only its two protected user-owned modifications.
