...
{
    internal sealed class Gate10GoldenResult
    {
        public Gate10GoldenResult(
    ...
    internal static class Gate10TickPacingGoldenVector
    {
        internal static Gate10GoldenResult RunTwoPlayer()
        {
            ...
            Record(
                ...
                states,
                digests);
            ProcessElapsed(pacer, 9L, simulation, batches, frames, states, digests);
            ProcessElapsed(pacer, 1L, simulation, batches, frames, states, digests);
            ProcessElapsed(pacer, 10L, simulation, batches, frames, states, digests);

            return CreateResult(publisher, simulation, batches, frames, states, digests);
        }

        internal static Gate10GoldenResult RunThreePlayer()
        {
            ActiveRoster roster = new ActiveRoster(new[]
            {
                new PlayerId(500UL),
            ...
            Record(
                ...
                states,
                digests);
            ProcessElapsed(pacer, 4L, simulation, batches, frames, states, digests);
            ProcessElapsed(pacer, 6L, simulation, batches, frames, states, digests);
            Record(
                publisher.Submit(
                    roster.GetPlayerId(new PlayerSlot(1)),
        ...
        internal static Gate10GoldenResult RunFourPlayerPrimary()
        {
        ...
        internal static Gate10GoldenResult RunFourPlayerAlternative()
        {
            return RunFourPlayer(
                new[] { 30L },
                new[] { 10L, 10L });
        ...
        private static Gate10GoldenResult RunFourPlayer(
            long[] elapsedBeforeGap,
            long[] elapsedAfterGap)
        {
            ...
            SubmitTick(publisher, roster, CreateFourPlayerInputs(101U), new[] { 3, 1, 0, 2 }, simulation, batches, frames, states, digests);
            SubmitTick(publisher, roster, CreateFourPlayerInputs(102U), new[] { 2, 0, 3, 1 }, simulation, batches, frames, states, digests);
            SubmitTick(publisher, roster, CreateFourPlayerInputs(103U), new[] { 1, 3, 0, 2 }, simulation, batches, frames, states, digests);
            SubmitTick(publisher, roster, CreateFourPlayerInputs(100U), new[] { 0, 2, 1 }, simulation, batches, frames, states, digests);
            ProcessElapsed(pacer, elapsedBeforeGap, simulation, batches, frames, states, digests);
            ...
            Record(
                ...
                states,
                digests);
            ProcessElapsed(pacer, elapsedAfterGap, simulation, batches, frames, states, digests);
        ...
        private static void ProcessElapsed(
        ...
        {
            Record(
                pacer.ProcessElapsedStopwatchTicks(elapsedStopwatchTicks),
                simulation,
                batches,
                frames,
                states,
        ...
        private static void ProcessElapsed(
            ...
            List<ulong> digests)
        {
            for (int index = 0; index < elapsedStopwatchTicks.Length; index++)
            {
                ProcessElapsed(
                    pacer,
                    elapsedStopwatchTicks[index],
                    simulation,
                    batches,
                    frames,
                    states,
        ...
        private static Gate10GoldenResult CreateResult(
            TickDrivenFramePublisher publisher,
            BattleSimulation simulation,
            List<FrameData[]> batches,
            List<FrameData> frames,
